...
Feature:
	...
	Scenario: Simulation 1
		...
		# NIGHT 1: the lupuses don't reach the quorum
		...
		| state.players.`user4_id`.alive | true |


		# DAY 2: the villagers don't reach the quorum

		When Player user1 voted `user4_id` in game `game_id`
		When Player user2 voted `user4_id` in game `game_id`
		When Player user3 voted `user4_id` in game `game_id`
		When Player user4 voted `user5_id` in game `game_id`
		...
		# DAY 4: the villagers kill a farmer
